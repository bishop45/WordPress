---
name: write-post
description: AdSense対応のブログ記事を自動生成・投稿します。記事構成の提案から本文執筆、WordPress投稿、画像設定、メタディスクリプション設定、バックアップまで一気通貫で実行。
...
このSkillは、記事の生成から投稿、画像設定、メタディスクリプション設定、バックアップまでを自動で行います。
